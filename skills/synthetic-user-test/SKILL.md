...
description: Run synthetic user testing sessions for any product — create personas, save them for reuse, and get focused reports. Use this skill when the user wants to test a feature or flow with synthetic users, conduct a heuristic evaluation, cognitive walkthrough, create or manage test personas, or simulate how different user types would experience a design. Triggers on "test this", "what would a user think", "synthetic user", "persona testing", "heuristic evaluation", "cognitive walkthrough", "usability test", "panel discussion", "create persona", "my personas", "delete persona", or any request to evaluate a design from a user's perspective.
---
...
## Standalone Persona Commands

These commands work OUTSIDE of a test session — the user can manage personas anytime:

**"Create a persona" / "new persona" / "build a persona":**
Go directly to Phase 3b (persona creation questions). After creation, ask to save. Do NOT start a test session unless the user asks.

**"Show my personas" / "list personas" / "my personas":**
Scan `knowledge/personas/` for all `.md` files (excluding README.md). Display:
> "Here are your saved personas:
> [Numbered list — Name, Role, Tech level]
>
> Want to create a new one, delete one, or start a test?"

If no personas found:
> "You don't have any saved personas yet. Want to create one?"

**"Delete [persona name]":**
Find the matching file in `knowledge/personas/`. Confirm before deleting:
